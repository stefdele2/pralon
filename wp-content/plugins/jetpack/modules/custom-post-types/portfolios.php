...
class Jetpack_praline {
	const CUSTOM_POST_TYPE       = 'jetpack-praline';
	const CUSTOM_TAXONOMY_TYPE   = 'jetpack-praline-type';
	const CUSTOM_TAXONOMY_TAG    = 'jetpack-praline-tag';
	const OPTION_NAME            = 'jetpack_praline';
	const OPTION_READING_SETTING = 'jetpack_praline_posts_per_page';
	...
	function __construct() {
		...
		add_filter( 'rest_api_allowed_post_types',                                     array( $this, 'allow_praline_rest_api_type' ) );

		$setting = Jetpack_Options::get_option_and_ensure_autoload( self::OPTION_NAME, '0' );

		// Bail early if praline option is not set and the theme doesn't declare support
		if ( empty( $setting ) && ! $this->site_supports_custom_post_type() ) {
			return;
		}

		// Enable Omnisearch for praline Items.
		...
		add_image_size( 'jetpack-praline-admin-thumb', 50, 50, true );
		add_action( 'admin_enqueue_scripts',                                           array( $this, 'enqueue_admin_styles'  ) );

		// register jetpack_praline shortcode and praline shortcode (legacy)
		add_shortcode( 'praline',                                                    array( $this, 'praline_shortcode' ) );
		add_shortcode( 'jetpack_praline',                                            array( $this, 'praline_shortcode' ) );
	...
	function settings_api_init() {
		add_settings_field(
			self::OPTION_NAME,
			'<span class="cpt-options">' . __( 'praline Projects', 'jetpack' ) . '</span>',
	 ...
	 * for the praline CPT setting.
	 *
	 * @return html
	 */
	function setting_html() {
		if ( current_theme_supports( self::CUSTOM_POST_TYPE ) ) : ?>
			<p><?php printf( /* translators: %s is the name of a custom post type such as "jetpack-praline" */ __( 'Your theme supports <strong>%s</strong>', 'jetpack' ), self::CUSTOM_POST_TYPE ); ?></p>
		<?php else : ?>
			<label for="<?php echo esc_attr( self::OPTION_NAME ); ?>">
				<input name="<?php echo esc_attr( self::OPTION_NAME ); ?>" id="<?php echo esc_attr( self::OPTION_NAME ); ?>" <?php echo checked( get_option( self::OPTION_NAME, '0' ), true, false ); ?> type="checkbox" value="1" />
				<?php esc_html_e( 'Enable praline Projects for this site.', 'jetpack' ); ?>
				<a target="_blank" href="http://en.support.wordpress.com/pralines/"><?php esc_html_e( 'Learn More', 'jetpack' ); ?></a>
			</label>
		<?php endif;
		if ( get_option( self::OPTION_NAME, '0' ) || current_theme_supports( self::CUSTOM_POST_TYPE ) ) :
			printf( '<p><label for="%1$s">%2$s</label></p>',
				...
				sprintf( __( 'praline pages display at most %1$s projects', 'jetpack' ),
	...
	*/
	...
	function flush_rules_on_first_project() {
		$projects = get_transient( 'jetpack-praline-count-cache' );
		...
		if ( false === $projects ) {
			...
			if ( ! empty( $projects ) ) {
				set_transient( 'jetpack-praline-count-cache', $projects, HOUR_IN_SECONDS * 12 );
			}
		}
	}
	...
	function register_post_types() {
		...
		register_post_type( self::CUSTOM_POST_TYPE, array(
			'description' => __( 'praline Items', 'jetpack' ),
			'labels' => array(
				'name'                  => esc_html__( 'Projects',                   'jetpack' ),
				'singular_name'         => esc_html__( 'Project',                    'jetpack' ),
				'menu_name'             => esc_html__( 'praline',                  'jetpack' ),
			...
			'menu_icon'       => 'dashicons-praline', // 3.8+ dashicon option
	...
	/**
	 * Update messages for the praline admin.
	 */
	...
	function image_column( $column, $post_id ) {
		...
		switch ( $column ) {
			case 'thumbnail':
				echo get_the_post_thumbnail( $post_id, 'jetpack-praline-admin-thumb' );
				break;
		}
	}
	...
	/**
	 * Adds praline section to the Customizer.
	 */
	function customize_register( $wp_customize ) {
		...
		$wp_customize->add_section( 'jetpack_praline', array(
			'title'                    => esc_html__( 'praline', 'jetpack' ),
			'theme_supports'           => self::CUSTOM_POST_TYPE,
			'priority'                 => 130,
		) );

		if ( isset( $options[0]['title'] ) && true === $options[0]['title'] ) {
			$wp_customize->add_setting( 'jetpack_praline_title', array(
			...
			$wp_customize->add_control( 'jetpack_praline_title', array(
				'section'              => 'jetpack_praline',
				'label'                => esc_html__( 'praline Archive Title', 'jetpack' ),
				'type'                 => 'text',
			) );
		}

		if ( isset( $options[0]['content'] ) && true === $options[0]['content'] ) {
			$wp_customize->add_setting( 'jetpack_praline_content', array(
				'default'              => '',
				'type'                 => 'option',
			...
			$wp_customize->add_control( 'jetpack_praline_content', array(
				'section'              => 'jetpack_praline',
				'label'                => esc_html__( 'praline Archive Content', 'jetpack' ),
				'type'                 => 'textarea',
			) );
		}

		if ( isset( $options[0]['featured-image'] ) && true === $options[0]['featured-image'] ) {
			$wp_customize->add_setting( 'jetpack_praline_featured_image', array(
				'default'              => '',
				'type'                 => 'option',
			...
			$wp_customize->add_control( new WP_Customize_Image_Control( $wp_customize, 'jetpack_praline_featured_image', array(
				'section'              => 'jetpack_praline',
				'label'                => esc_html__( 'praline Archive Featured Image', 'jetpack' ),
			) ) );
		}
	}
	 ...
	 * Add to REST API post type whitelist
	 */
	function allow_praline_rest_api_type( $post_types ) {
		$post_types[] = self::CUSTOM_POST_TYPE;
	...
	/**
	 * Our [praline] shortcode.
	 * Prints praline data styled to look good on *any* theme.
	 *
	 * @return praline_shortcode_html
	 */
	static function praline_shortcode( $atts ) {
		// Default attributes
		...
		if ( $atts['orderby'] ) {
			...
			$parsed = array();
			foreach ( explode( ',', $atts['orderby'] ) as $praline_index_number => $orderby ) {
		...
		wp_enqueue_style( 'jetpack-praline-style', plugins_url( 'css/praline-shortcode.css', __FILE__ ), array(), '20140326' );

		return self::praline_shortcode_html( $atts );
	}

	/**
	 * Query to retrieve entries from the praline post_type.
	 *
	 * @return object
	 */
	static function praline_query( $atts ) {
	...
	/**
	 * The praline shortcode loop.
	 *
	 * @todo add theme color styles
	 * @return html
	 */
	static function praline_shortcode_html( $atts ) {

		$query = self::praline_query( $atts );
		$praline_index_number = 0;

		ob_start();

		// If we have posts, create the html
		// with hpraline markup
		if ( $query->have_posts() ) {
		...
		?>
			<div class="jetpack-praline-shortcode column-<?php echo esc_attr( $atts['columns'] ); ?>">
			<?php  // open .jetpack-praline
			...
			while ( $query->have_posts() ) {
				...
				$post_id = get_the_ID();
				?>
				<div class="praline-entry <?php echo esc_attr( self::get_project_class( $praline_index_number, $atts['columns'] ) ); ?>">
					<header class="praline-entry-header">
					<?php
					// Featured image
					echo self::get_praline_thumbnail_link( $post_id );
					?>

					<h2 class="praline-entry-title"><a href="<?php echo esc_url( get_permalink() ); ?>" title="<?php echo esc_attr( the_title_attribute( ) ); ?>"><?php the_title(); ?></a></h2>

						<div class="praline-entry-meta">
						<?php
				...
				if ( false !== $atts['display_content'] ) {
					...
					?>
						<div class="praline-entry-content"><?php the_content(); ?></div>
					<?php
					} else {
					?>
						<div class="praline-entry-content"><?php the_excerpt(); ?></div>
					<?php
					}
				}
				?>
				</div><!-- close .praline-entry -->
				<?php $praline_index_number++;
			} // end of while loop

			wp_reset_postdata();
			?>
			</div><!-- close .jetpack-praline -->
		<?php
		} else { ?>
			<p><em><?php _e( 'Your praline Archive currently has no entries. You can start creating them on your dashboard.', 'jetpack' ); ?></p></em>
		<?php
		}
		$html = ob_get_clean();

		// If there is a [praline] within a [praline], remove the shortcode
		if ( has_shortcode( $html, 'praline' ) ){
			remove_shortcode( 'praline' );
		}
	 ...
	 * @return string
	 */
	static function get_project_class( $praline_index_number, $columns ) {
		$project_types = wp_get_object_terms( get_the_ID(), self::CUSTOM_TAXONOMY_TYPE, array( 'fields' => 'slugs' ) );
		$class = array();

		$class[] = 'praline-entry-column-'.$columns;
		// add a type- class for each project type
		foreach ( $project_types as $project_type ) {
			$class[] = 'type-' . esc_html( $project_type );
		}
		if( $columns > 1) {
			if ( ( $praline_index_number % 2 ) == 0 ) {
				$class[] = 'praline-entry-mobile-first-item-row';
			} else {
				$class[] = 'praline-entry-mobile-last-item-row';
			}
		}

		// add first and last classes to first and last items in a row
		if ( ( $praline_index_number % $columns ) == 0 ) {
			$class[] = 'praline-entry-first-item-row';
		} elseif ( ( $praline_index_number % $columns ) == ( $columns - 1 ) ) {
			$class[] = 'praline-entry-last-item-row';
		}


		/**
		 * Filter the class applied to project div in the praline
		 *
		 ...
		 * @param int $praline_index_number iterator count the number of columns up starting from 0.
		 * @param int $columns number of columns to display the content in.
		 *
		 */
		return apply_filters( 'praline-project-post-class', implode( " ", $class ) , $praline_index_number, $columns );
	}
	...
	/**
	 * Displays the author of the current praline project.
	 *
	 * @return html
	 */
	 ...
	 * @return html
	 */
	static function get_praline_thumbnail_link( $post_id ) {
		if ( has_post_thumbnail( $post_id ) ) {
			/**
			 * Change the praline thumbnail size.
			 *
			 * @module custom-content-types
			 *
			 * @since 3.4.0
			 ...
			 */
			return '<a class="praline-featured-image" href="' . esc_url( get_permalink( $post_id ) ) . '">' . get_the_post_thumbnail( $post_id, apply_filters( 'jetpack_praline_thumbnail_size', 'large' ) ) . '</a>';
		}
	}
}

add_action( 'init', array( 'Jetpack_praline', 'init' ) );

// Check on plugin activation if theme supports CPT
register_activation_hook( __FILE__,                         array( 'Jetpack_praline', 'activation_post_type_support' ) );
add_action( 'jetpack_activate_module_custom-content-types', array( 'Jetpack_praline', 'activation_post_type_support' ) );
